...
## 11. Overview — UI クラスタリング（post-Phase-0 追記）

> 2026-09-09 追記（README compression pass 2）。`ui` の Overview 画面のクラスタリング仕様は本設計文書の初稿時点ではまだ実装されておらず、根拠だけ現行 `README.md` 側に書かれていた。Full README 概念の廃止に伴い、design rationale の一部としてここへ着地させる。

`ui` の最初の画面は Findings の羅列ではなく、**平易な言葉での症状要約**にする。

- 見出しは **`N areas need attention`。N は Finding の数ではなく、クラスタリング後の root cause の数**（実例: 19 Findings → 6 areas）
- **クラスタリングは、原因を束ねる観測された事実がある時だけ許可される。** 束ねる鍵は「同じ plugin namespace を指している」「同じ runtime ペアで同名だが内容が違う」「同じ runtime で同種の drift が起きている」「同じコマンドが登録されている」など。安全に束ねられないものはそのまま単独項目として残る。**見た目を整えるためだけの推測クラスタリングはしない** — どのクラスタにも必ず「なぜこう束ねたか」が表示される
- 隣には **「問題として診断しなかったもの」**が並ぶ: 固定の起動コストの合計、保護対象の資源、発火記録のない skill、別 runtime の領域にあることが分かっている不可視、評価していないチェック、見ていない期間。**Doctor が Optimizer ではないことを、毎回ここで証明する**
- Claude と Codex で見えているものの違い、直近の変更（観測していない期間を「変化なし」として扱わない）は同じ画面に載る
- Structure では、各資源の詳細の前に **「これはどういう意味か」を 1〜2 行**で先に出す。例: 「Claude はこのファイルを見ている。Codex は `~/.agents/skills/…` を見ている。同名だが別コピーで、内容が 8 行違う。」**事実の平易な翻訳だけで、直し方の提案もどちらが正かの判定もしない**

Structure / Findings / Context cost / Hooks / History / Not findings / Coverage という専門家向けの層は消えない。Overview はそこへの入口であって、置き換えではない。**Not findings** は特に、「見た上で症状と判断しなかった」ことの証明そのものなので残る。
